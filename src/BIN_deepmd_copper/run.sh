...
# (cp lmp_execute $outfile/) &&  (cp lmp_deepmd_pjsub $outfile/) &&  (cp copper.lmp $outfile/) &&  (cp in.deepmd $outfile/)  && \
#          (cd $outfile && pjsub --interact --sparam wait-time=600 lmp_deepmd_pjsub)
(cp lmp_execute_* $outfile/) &&  (cp lmp_deepmd_pjsub $outfile/) &&  (cp copper.lmp $outfile/) &&  (cp in.deepmd_* $outfile/) &&  (cp pb_data_copper.dat $outfile/) && \
