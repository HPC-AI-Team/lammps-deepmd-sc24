     ...
     efield_force_flag(0)
{
  ...
  utils::logmesg_arry(lmp, "[INFO] fix_dplr map_vec", map_vec.data(), map_vec.size(), 1);
  utils::logmesg_arry(lmp, "[INFO] fix_dplr sel_type", sel_type.data(), sel_type.size(), 1);
...
void FixDPLR::post_integrate()
{
  ...
  // if (comm->me == 0) utils::logmesg(lmp,"[INFO] FixDPLR::post_integrate \n");
...
void FixDPLR::post_force(int vflag)
{
  ...
  // compute
  printf("[INFO ]FixDPLR::post_force %d \n", 0); fflush(stdout);
